...
data, Unggah,preporcessing, modeling, implementation = st.tabs(["Data", "Unggah","Prepocessing", "Modeling", "Implementation"])
...
with preporcessing:
    ...
    y = df.stroke
    showdf = st.button("Data yang dibuang")
    if showdf:
    ...
    showtr = st.button("Transformasi label")
    if showtr:
        "### Transformasi Label"
        y
    ...
    showlabel = st.button("Label")
    if showlabel:
        "### Label"
        labels

    shownor = st.button("Normalisasi")
    if shownor:
    ...
    shownordn = st.button("Normalisasi Data Transformasi")
    if shownordn:
    ...
    showminmax = st.button("Normalisasi MinMax Scaler")
    if showminmax:
